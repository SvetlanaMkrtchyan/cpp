...
static string trimCmd(const string& s) {
...
string formatInst(int idx, const Instruction& inst, const vector<double>& constPool,
    ...
    case OpCode::JMP_REL:      ss << "-> pc+=" << signExt(inst.resIdx, 9); break;
    case OpCode::JMP_IF_FALSE: ss << "r" << inst.resIdx << ", -> pc=" << inst.leftIdx; break;
    case OpCode::JMP_IF_TRUE:  ss << "r" << inst.resIdx << ", -> pc+=" << signExt(inst.leftIdx, 9); break;
    ...
    case OpCode::BR_LTE: case OpCode::BR_LTE_U:
        ss << "r" << inst.resIdx << ", r" << inst.leftIdx << ", offset=" << signExt(inst.rightIdx, 8); break;
    ...
    case OpCode::CALL_STK:
        ss << "func[" << inst.leftIdx << "], locals=" << inst.rightIdx; break;
    case OpCode::RET_STK:
        ss << "r" << inst.resIdx << ", args=" << inst.leftIdx; break;
    case OpCode::PUSH_BP:  ss << ""; break;
    case OpCode::MOV_BP_SP: ss << ""; break;
    case OpCode::POP_BP:   ss << ""; break;
    case OpCode::SUB_SP:   ss << inst.resIdx << "*8"; break;
    case OpCode::PUSH_ARG: ss << "r" << inst.resIdx; break;
    case OpCode::LOAD_BP_OFF:  ss << "r" << inst.resIdx << ", [BP+" << (int32_t)inst.leftIdx << "]"; break;
    case OpCode::STORE_BP_OFF: ss << "[BP+" << (int32_t)inst.leftIdx << "], r" << inst.resIdx; break;
    case OpCode::READ_INT:    ss << "r" << inst.resIdx; break;
    case OpCode::READ_DOUBLE: ss << "r" << inst.resIdx; break;
    case OpCode::PRINT:       ss << "r" << inst.resIdx; break;
    case OpCode::RET:
    case OpCode::HALT:
...
void runDebugger(
    ...
    cout << "\n[Debugger] Commands (type at dbg> prompt only):\n";
    cout << "  s          step one instruction\n";
    cout << "  c          continue until breakpoint or end\n";
    cout << "  b <n>      set breakpoint at instruction n  (e.g. b 3)\n";
    cout << "  d <n>      remove breakpoint at n\n";
    cout << "  r          show registers\n";
    cout << "  v          show variables\n";
    cout << "  l          list all instructions\n";
    cout << "  q          quit debugger\n";
    cout << "  (Set breakpoints with b before using c)\n\n";
    ...
    while (pc < (int)prog.size()) {
        ...
        if (stepping || atBreak) {
            ...
            if (verb == "q" || verb == "quit") break;
            if (verb == "s" || verb.empty()) {
                stepping = true;
            } else if (verb == "c" || verb == "continue") {
                stepping = false;
            } else if (verb == "r" || verb == "regs") {
                printRegs();
                continue;
            } else if (verb == "v" || verb == "vars") {
                printVars();
                continue;
            } else if (verb == "l" || verb == "list") {
                printListing();
                continue;
            } else if (verb == "b" || verb == "break") {
                if (argN < 0) {
                    cout << "Usage: b <instruction_number>   (e.g. b 3)\n";
                    continue;
                }
                breakpoints.insert(argN);
                cout << "Breakpoint set at instruction " << argN << "\n";
                continue;
            } else if (verb == "d" || verb == "del") {
                if (argN < 0) {
                    cout << "Usage: d <instruction_number>\n";
                    continue;
                }
                breakpoints.erase(argN);
                cout << "Breakpoint " << argN << " removed\n";
                continue;
            } else if (!line.empty()) {
                cout << "Unknown debugger command: \"" << line
                     << "\". Type s, c, b <n>, r, v, l, or q.\n";
                continue;
            }
        }
        ...
        case OpCode::JMP_REL:  pc += signExt(inst.resIdx, 9); break;
        case OpCode::JMP_IF_FALSE: if (regs[inst.resIdx] == 0.0) pc = inst.leftIdx; break;
        case OpCode::JMP_IF_TRUE:  if (regs[inst.resIdx] != 0.0) pc += signExt(inst.leftIdx, 9); break;
        case OpCode::BR_GT:    if (regs[inst.resIdx] >  regs[inst.leftIdx]) pc += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_LT:    if (regs[inst.resIdx] <  regs[inst.leftIdx]) pc += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_EQ:    if (regs[inst.resIdx] == regs[inst.leftIdx]) pc += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_GTE:   if (regs[inst.resIdx] >= regs[inst.leftIdx]) pc += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_LTE:   if (regs[inst.resIdx] <= regs[inst.leftIdx]) pc += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_GT_U:  if ((uint32_t)regs[inst.resIdx] >  (uint32_t)regs[inst.leftIdx]) pc += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_LT_U:  if ((uint32_t)regs[inst.resIdx] <  (uint32_t)regs[inst.leftIdx]) pc += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_GTE_U: if ((uint32_t)regs[inst.resIdx] >= (uint32_t)regs[inst.leftIdx]) pc += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_LTE_U: if ((uint32_t)regs[inst.resIdx] <= (uint32_t)regs[inst.leftIdx]) pc += signExt(inst.rightIdx, 8); break;
        case OpCode::JA:
        case OpCode::JAE:
        case OpCode::JB:
        case OpCode::JBE:
...
double executeWithStack(
    ...
    auto stackPushDouble = [&](double val) {
        cpu.sp -= 8;
        memory.writeDouble(cpu.sp, val);
    };

    auto stackPopDouble = [&]() -> double {
        double val = memory.readDouble(cpu.sp);
        cpu.sp += 8;
        return val;
    };

    while ((int)cpu.ip < (int)prog.size()) {
        Instruction inst = prog[(int)cpu.ip];
        cpu.ip++;
        ...
        case OpCode::LOAD_BP_OFF: {
            int32_t offset = (int32_t)inst.leftIdx;
            regs[inst.resIdx] = memory.readDouble(cpu.bp + offset);
            break;
        }
        case OpCode::STORE_BP_OFF: {
            int32_t offset = (int32_t)inst.leftIdx;
            memory.writeDouble(cpu.bp + offset, regs[inst.resIdx]);
            break;
        }
        case OpCode::PUSH_BP:
            stackPushDouble((double)cpu.bp);
            break;
        case OpCode::MOV_BP_SP:
            cpu.bp = cpu.sp;
            break;
        case OpCode::POP_BP:
            cpu.bp = (uint64_t)stackPopDouble();
            break;
        case OpCode::SUB_SP:
            cpu.sp -= (uint32_t)inst.resIdx * sizeof(double);
            break;
        case OpCode::PUSH_ARG:
            stackPushDouble(regs[inst.resIdx]);
            break;
        case OpCode::CALL_STK: {
            stackPushDouble((double)cpu.ip);
            stackPushDouble((double)cpu.bp);
            cpu.bp = cpu.sp;
            int numLocals = (int)inst.rightIdx;
            cpu.sp -= numLocals * sizeof(double);
            cpu.ip = inst.leftIdx;
            break;
        }
        case OpCode::RET_STK: {
            ...
            cpu.sp = cpu.bp;
            cpu.bp = (uint64_t)stackPopDouble();
            cpu.ip = (uint64_t)stackPopDouble();
            int numArgs = (int)inst.leftIdx;
            cpu.sp += numArgs * sizeof(double);
            regs[inst.resIdx] = retVal;
            return retVal;
        }
        case OpCode::ADD:      regs[inst.resIdx] = regs[inst.leftIdx] + regs[inst.rightIdx]; break;
        case OpCode::SUB:      regs[inst.resIdx] = regs[inst.leftIdx] - regs[inst.rightIdx]; break;
        ...
        case OpCode::CMP_GTE:  updateFlagsFromCompare(regs[inst.leftIdx], regs[inst.rightIdx], flags); regs[inst.resIdx] = regs[inst.leftIdx] >= regs[inst.rightIdx] ? 1.0 : 0.0; break;
        case OpCode::CMP_FLAGS: updateFlagsFromCompare(regs[inst.resIdx], regs[inst.leftIdx], flags); break;
        case OpCode::JMP:      cpu.ip = inst.resIdx; break;
        case OpCode::JMP_REL:  cpu.ip += signExt(inst.resIdx, 9); break;
        case OpCode::JMP_IF_FALSE: if (regs[inst.resIdx] == 0.0) cpu.ip = inst.leftIdx; break;
        case OpCode::JMP_IF_TRUE:  if (regs[inst.resIdx] != 0.0) cpu.ip += signExt(inst.leftIdx, 9); break;
        case OpCode::BR_GT:    if (regs[inst.resIdx] >  regs[inst.leftIdx]) cpu.ip += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_LT:    if (regs[inst.resIdx] <  regs[inst.leftIdx]) cpu.ip += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_EQ:    if (regs[inst.resIdx] == regs[inst.leftIdx]) cpu.ip += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_GTE:   if (regs[inst.resIdx] >= regs[inst.leftIdx]) cpu.ip += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_LTE:   if (regs[inst.resIdx] <= regs[inst.leftIdx]) cpu.ip += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_GT_U:  if ((uint32_t)regs[inst.resIdx] >  (uint32_t)regs[inst.leftIdx]) cpu.ip += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_LT_U:  if ((uint32_t)regs[inst.resIdx] <  (uint32_t)regs[inst.leftIdx]) cpu.ip += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_GTE_U: if ((uint32_t)regs[inst.resIdx] >= (uint32_t)regs[inst.leftIdx]) cpu.ip += signExt(inst.rightIdx, 8); break;
        case OpCode::BR_LTE_U: if ((uint32_t)regs[inst.resIdx] <= (uint32_t)regs[inst.leftIdx]) cpu.ip += signExt(inst.rightIdx, 8); break;
        case OpCode::JA:
        case OpCode::JAE:
        case OpCode::JB:
        case OpCode::JBE:
        case OpCode::JE:
        case OpCode::JNE:
            if (shouldTakeFlagsJump((OpCode)inst.op, flags)) cpu.ip += (int8_t)inst.rightIdx;
            break;
        case OpCode::READ_INT:    { int v; cin >> v; regs[inst.resIdx] = v; break; }
        case OpCode::READ_DOUBLE: { double v; cin >> v; regs[inst.resIdx] = v; break; }
        case OpCode::PRINT:       cout << regs[inst.resIdx] << "\n"; break;
        case OpCode::CALL: {
            auto& fe = funcTable[inst.leftIdx];
            int numArgs = (int)fe.params.size();
            uint64_t argsBase = cpu.sp;
            stackPushDouble((double)cpu.ip);
            stackPushDouble((double)cpu.bp);
            cpu.bp = cpu.sp;
            vector<double> localVars(max(numArgs + 32, 64), 0.0);
            for (int i = 0; i < numArgs; i++)
                localVars[i] = memory.readDouble(argsBase + i * sizeof(double));
            double innerRegs[NUM_REGS] = {};
            for (int i = 0; i < numArgs; i++) innerRegs[i + 1] = localVars[i];
            double retVal = execute(fe.code, fe.constPool, localVars, innerRegs, 0);
            cpu.bp = (uint64_t)stackPopDouble();
            cpu.ip = (uint64_t)stackPopDouble();
            cpu.sp += numArgs * sizeof(double);
            regs[inst.resIdx] = retVal;
...
double executeFlat(
    ...
    while (ip < (int)code.size()) {
        ...
        case OpCode::RET: {
            if (callStack.empty()) return regs[inst.resIdx];
            double retVal = regs[inst.resIdx];
