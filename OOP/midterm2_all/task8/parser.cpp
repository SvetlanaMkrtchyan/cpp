...
static void expect(bool cond, const string& msg) {
    if (!cond) throw runtime_error("Parse error: " + msg);
}
...
static unique_ptr<ExprNode> parseLogicalAnd() {
    auto l = parseBitOr();
    while (!atEnd() && peekVal("&&")) {
        consume();
        auto p = make_unique<ExprNode>(NodeKind::OP_NODE, "&&");
...
static unique_ptr<ExprNode> parseLogicalOr() {
    auto l = parseLogicalAnd();
    while (!atEnd() && peekVal("||")) {
        consume();
        auto p = make_unique<ExprNode>(NodeKind::OP_NODE, "||");
        p->left  = move(l);
        p->right = parseLogicalAnd();
        l = move(p);
    }
    return l;
}

static unique_ptr<ExprNode> parseUnary() {
    if (peekVal("!")) {
        consume();
        auto node = make_unique<ExprNode>(NodeKind::NOT_NODE, "!");
...
unique_ptr<ExprNode> parseExpr() { return parseUnary(); }
...
unique_ptr<ExprNode> parseStatement() {
    ...
    if (peek(KW_CONTINUE)) {
        consume();
        if (peek(SEMICOLON)) consume();
        return make_unique<ExprNode>(NodeKind::CONTINUE_NODE, "continue");
    }

    if (peek(KW_PRINT)) {
        consume();
        expect(peek(LPAREN), "expected '(' after print");
        consume();
        auto node = make_unique<ExprNode>(NodeKind::PRINT_NODE, "print");
        node->left = parseExpr();
        expect(peek(RPAREN), "expected ')' after print argument");
        consume();
        if (peek(SEMICOLON)) consume();
        return node;
    }
    ...
    if (peek(KW_FOR)) {
        consume();
        if (peek(LPAREN)) consume();
        unique_ptr<ExprNode> init;
        if (peek(KW_INT) || peek(KW_DOUBLE))
            init = parseStatement();
        else
            init = parseAssignOrExpr();
        if (peek(SEMICOLON)) consume();
        auto cond = parseExpr();
        if (peek(SEMICOLON)) consume();
