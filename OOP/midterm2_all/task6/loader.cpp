              ...
              int                        entryPoint)
{
    ...
    hdr.codeSize   = (uint32_t)flatCode.size();
    hdr.dataSize   = (uint32_t)varNames.size();
    hdr.constSize  = (uint32_t)flatConst.size();
...
ExecFile loadExec(const string& path) {
    ...
    f.read(reinterpret_cast<char*>(ef.code.data()),
           ef.header.codeSize * sizeof(Instruction));

    ef.constPool.resize(ef.header.constSize);
    f.read(reinterpret_cast<char*>(ef.constPool.data()),
           ef.header.constSize * sizeof(double));
